    ...
    if os.path.exists(FAJL_BAZA):
        try:
            with open(FAJL_BAZA, "r", encoding="utf-8") as f:
                база = json.load(f)
                # Конверзија на старата база во нов формат ако е потребно
                for испит, инфо in база.items():
                    for глава, г_инфо in инфо["glavi"].items():
                        if "sodrzina" in g_инfo:  # Стар формат
                            стара_содржина = г_инфо.pop("sodrzina")
                            стар_статус = г_инфо.pop("status", "🔴 Недопрено")
                            г_инфо["temi"] = {
                                "Општа тема": {"sodrzina": стара_содржина, "status": стар_статус}
                            }
                return база
        except:
            pass
...
if st.sidebar.button("➕ Креирај испит"):
    if нов_испит.strip() and нов_испит not in st.session_state.ispiti:
        st.session_state.ispiti[нов_испит.strip()] = {
            ...
            "glavi": {"Глава 1": {"temi": {"Вовед": {"sodrzina": "", "status": "🔴 Недопрено"}}}}
        }
        zacuvaj_podatoci()
        st.rerun()
...
if st.session_state.ispiti:
    ...
    # Менаџирање со Глави
    st.sidebar.subheader("📂 Глави на предметот")
    нова_глава_име = st.sidebar.text_input("Име на нова Глава:")
    if st.sidebar.button("➕ Додај Глава"):
        if нова_глава_име.strip() and нова_глава_име.strip() not in podatoci["glavi"]:
            podatoci["glavi"][нова_глава_име.strip()] = {"temi": {"Почетна тема": {"sodrzina": "", "status": "🔴 Недопрено"}}}
            zacuvaj_podatoci()
            st.rerun()
            
    листа_глави = list(podatoci["glavi"].keys())
    избрана_глава = st.sidebar.selectbox("Избери Глава:", листа_глави)
    ...
    if st.sidebar.button("🗑️ Избриши ја оваа глава", type="primary", key="del_glava"):
        if len(podatoci["glavi"]) > 1:
            podatoci["glavi"].pop(избрана_глава)
            zacuvaj_podatoci()
            st.rerun()

    st.sidebar.markdown("---")
    
    # НОВО: Менаџирање со Теми (Pages) во рамки на избраната глава
    st.sidebar.subheader("📄 Теми (Страници) во Главата")
    нова_тема_име = st.sidebar.text_input("Име на нова Тема (Page):")
    if st.sidebar.button("➕ Додај Тема"):
        if нова_тема_име.strip() and нова_тема_име.strip() not in podatoci["glavi"][избрана_глава]["temi"]:
            podatoci["glavi"][избрана_глава]["temi"][нова_тема_име.strip()] = {"sodrzina": "", "status": "🔴 Недопрено"}
            zacuvaj_podatoci()
            st.rerun()
            
    листа_теми = list(podatoci["glavi"][избрана_глава]["temi"].keys())
    избрана_тема = st.sidebar.radio("Избери активна Тема:", листа_теми)
    
    ново_име_тема = st.sidebar.text_input("Преименувај ја темата:", value=избрана_тема)
    if st.sidebar.button("💾 Зачувај име на тема"):
        if ново_име_тема.strip() and ново_име_тема.strip() != избрана_тема:
            podatoci["glavi"][избрана_глава]["temi"][ново_име_тема.strip()] = podatoci["glavi"][избрана_глава]["temi"].pop(избрана_теma)
            zacuvaj_podatoci()
            st.rerun()
            
    if st.sidebar.button("🗑️ Избриши ја оваа тема", type="primary", key="del_tema"):
        if len(podatoci["glavi"][избрана_глава]["temi"]) > 1:
            podatoci["glavi"][избрана_глава]["temi"].pop(избрана_тема)
            zacuvaj_podatoci()
            st.rerun()

    st.sidebar.markdown("---")
    # Глобални контроли за предмети
    ново_име = st.sidebar.text_input("Преименувај го целиот предмет:", value=избран_испит)
...
if st.session_state.ispiti:
    ...
    st.caption(f"📁 Глава: {избрана_глава}")
    
    # Тековни податоци за избраната тема
    тековна_тема_податоци = podatoci["glavi"][избрана_глава]["temi"][избрана_тема]
    
    col_h, col_s = st.columns([2, 1])
    with col_h:
        st.subheader(f"📄 Страница / Тема: {избрана_тема}")
    with col_s:
        тековен_status = тековна_тема_податоци.get("status", "🔴 Недопрено")
        избран_status = st.selectbox("Статус на тема:", ["🔴 Недопрено", "🟡 Се учи", "🟢 Научено"], index=["🔴 Недопрено", "🟡 Се учи", "🟢 Научено"].index(тековен_status))
        if избран_status != тековен_status:
            тековна_тема_податоци["status"] = избран_status
            zacuvaj_podatoci()

    # Поле за пишување содржина
    тековна_содржина = тековна_тема_податоци["sodrzina"]
    nova_sodrzina = st.text_area("Внесете ја содржината за оваа тема:", value=тековна_содржина, height=200, key=f"area_{избран_испит}_{избрана_глава}_{избрана_тема}")
    if nova_sodrzina != тековна_содржина:
        тековна_тема_податоци["sodrzina"] = nova_sodrzina
        zacuvaj_podatoci()
    ...
    # ГЕНЕРИРАЊЕ WORD СО СИТЕ ГЛАВИ И НОВИТЕ ТЕМИ
    if st.button(f"🎬 Креирај комплетна скрипта за {избран_испит}"):
        ...
        for наслов_глава, инфо_глава in podatoci["glavi"].items():
            ...
            # Прошетка низ сите теми (Pages) на оваа глава
            for наслов_тема, инфо_тема in инфо_глава["temi"].items():
                t_heading = doc.add_heading(f"📄 Тема: {наслов_тема}", level=2)
                make_heading_collapsed_by_default(t_heading)
                
                p_status = doc.add_paragraph()
                p_status.add_run(f"Статус на темата: {инфо_тема.get('status', '🔴 Недопрено')}\n").italic = True
                
                редови = инфо_тема["sodrzina"].split('\n')
                for ред in редови:
                    ...
                    if ред.strip().startswith('>>'):
                        ...
                        procesiraj_tekst_so_stilovi(p, re.sub(r'^\d+\.\s', '', ред.strip()), recnik_poimi, избран_испит)
                    else:
                        p = doc.add_paragraph()
        ...
        doc.save(bio)
        bio.seek(0)
        st.success("🎉 Скриптата со сите поглавја и подтеми е генерирана!")
